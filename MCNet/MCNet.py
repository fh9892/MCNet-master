...
parser.add_argument(
    '-d', '--dataset', dest='dataset', default='KSC', help="Name of dataset.")
parser.add_argument(
    '-o',
    '--optimizer',
    dest='optimizer',
...
parser.add_argument(
    '-i', '--iter', type=int, dest='iter', default=5, help="No of iter")
parser.add_argument(
...
PARAM_DATASET = args.dataset  # UP,IN,SV, KSC,LK
...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'BW':
        BW = sio.loadmat(data_path + 'Botswana.mat')
        gt_BW = sio.loadmat(data_path + 'Botswana_gt.mat')
        data_hsi = BW['Botswana']
        gt_hsi = gt_BW['Botswana_gt']
        K = data_hsi.shape[2]
        TOTAL_SIZE = 3248
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
...
class ChannelAttention(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.ex = nn.Conv3d(channel // reduction, channel, 1, bias=False)
    ...
    def forward(self, x):
        ...
        max_out_identity = max_out
        avg_out_identity = avg_out

        max_out_1 = self.ex(max_out)
        max_out_2 = self.ex(max_out_identity)

        avg_out_1 = self.ex(avg_out)
        avg_out_2 = self.ex(avg_out_identity)

        output1 = self.sigmoid(max_out_1 + avg_out_1)
        output2 = self.sigmoid(max_out_2 + avg_out_2)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        max_result = max_result.unsqueeze(dim=4)
        avg_result = avg_result.unsqueeze(dim=4)
        result = torch.cat([max_result, avg_result], 1)
        result_identity = result

        output1 = self.conv(result)
        output2 = self.conv(result_identity)
...
class CBAMBlock(nn.Module):
    ...
    def __init__(self, channel, reduction=2):
        super().__init__()
        self.ca = ChannelAttention(channel=PARAM_KERNEL_SIZE, reduction=reduction)
...
class Residual(nn.Module):  # pytorch
    def __init__(
        ...
        # self.cbamlayer = CBAMBlock(out_channels)
...
class MCNet(nn.Module):
    ...
    def forward(self, X):
        ...
        Channel_attention_1, Channel_attention_2 = self.ca(U)
        Channel_attention_vector = torch.cat(
            [
                Channel_attention_1.unsqueeze(dim=1),
                Channel_attention_2.unsqueeze(dim=1)
            ],
            dim=1)
        V = (x1 * Channel_attention_vector).sum(dim=1)

        Spatial_attention_1, Spatial_attention_2 = self.sa(V)
        Spatial_attention_vector = torch.cat(
            [
                Spatial_attention_1.unsqueeze(dim=1),
                Spatial_attention_2.unsqueeze(dim=1)
            ],
            dim=1)

        W = (x1_raw * Spatial_attention_vector).sum(dim=1)
...
model = MCNet(BAND, CLASSES_NUM, 2).cuda()
...
for index_iter in range(ITER):
    ...
    torch.save(
        net.state_dict(), "./models/MCNetpatch_" + str(img_rows) + '_' +
...
record.record_output(
    ...
    './report/' + 'MCNetpatch:' + str(img_rows) + '_' + Dataset + 'split'
...
Utils.generate_png(
    ...
    './classification_maps/' + 'MCNetpatch:' + str(img_rows) + '_' +
